...
const PlanCard: React.FC<PlanCardProps> = ({ plan, index, isSelected, onSelect }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-card p-8 rounded-xl border-2 hover:shadow-2xl transition-all h-full flex flex-col relative cursor-pointer ${
        isSelected
          ? "border-primary shadow-primary/30 ring-2 ring-primary ring-opacity-50"
          : plan.isPopular
    ...
    >
      ...
      <div className="text-4xl font-extrabold mb-8 flex items-end">
        ${plan.price}
        <span className="text-xl font-medium text-muted-foreground ml-1">/{plan.frequency.slice(0, 3)}</span>
      </div>
...
function PlanesDeServicio() {
  ...
  return (
    <section id="planes" className="py-20 bg-muted/10">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {selectedPlanId && (
          <motion.div
            ...
            className="mt-12 p-6 bg-card rounded-xl border border-primary max-w-2xl mx-auto"
          >
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <Button className="bg-primary text-white hover:bg-primary/90 px-8">
                Proceder al pago
                <ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            </div>
